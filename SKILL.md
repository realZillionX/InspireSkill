...
| 观察收尾 | Events 看调度，Logs 看程序（Job / HPC / Ray / Serving 都有，Notebook 用 `exec` / `shell` 读），Metrics / Instances 看实际工作单元，Status 看平台状态和落点；训练曲线本身用 TensorBoard（`inspire tensorboard`）读。最后核验业务健康和产物，再清理资源。 |
