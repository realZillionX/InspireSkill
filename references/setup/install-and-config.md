...
安装、更新、账号配置和项目初始化看这一份。本机需要通过 Clash Verge 转发 `*.sii.edu.cn` 时，再加载 [`sii-proxy.md`](sii-proxy.md)。平台任务运行看 notebook / compute workloads / resources 等业务手册；命令表面以 CLI help 为准。
