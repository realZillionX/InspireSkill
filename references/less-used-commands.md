...
| `inspire serving metrics <id>` | 多副本部署的资源视图时间序列，和 `inspire notebook/job/hpc metrics` 同 UX（默认出 PNG 到 `~/.inspire/metrics/serving-<id>-<unix>.png` + per-replica stats + straggler 检测）；看哪个 replica 吃显存更多、是否均匀负载。lcg 自动从 `inference_servings/detail` 解。 |
