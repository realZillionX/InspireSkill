...
- 受限与否读机器上的 `nvidia-smi`，走同一条 Jupyter Terminal 通道；显卡不是 `H100` / `H200` 的 Notebook 可以建立本地 Connection，供 `ssh`、`scp`、`ssh-config` 和外部 OpenSSH 工具复用。结果按 Compute Group 存 `~/.inspire/notebook-gpu-models.json`（组内机器同型号），组名缺失时退回按 `notebook_id` 存。机器无应答时不猜 Transport，直接报错退出。
