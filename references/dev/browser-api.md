...
| 方法 | 路径 | 用途 | CLI 引用 |
| --- | --- | --- | --- |
| `POST` | `/api/v2/train?Action=CreateJobConsole` | 创建分布式训练任务。body 是前端提交体：顶层 `name`、`command`、`framework`、`project_id`、`workspace_id`、`logic_compute_group_id`、`task_priority`、`framework_config[]`，可选 `max_running_time_ms`（毫秒；后端列是 32-bit INT，上限约 `2.1e9` ms≈24 天，超出会以 MySQL `1264 (22003) Out of range` 报错——省略则不设运行时限，与多数 web UI 任务一致，CLI 默认省略）和 `exclude_nodes`。`framework_config[0]` 含 `image_type`、`image`、`instance_count`、`cpu`、`gpu_count`、`mem_gi`、`resource_spec_price:{cpu_type,cpu_count,gpu_type,gpu_count,memory_size_gib,logic_compute_group_id,quota_id}`，可选 `shm_gi`。quota 由顶层 `logic_compute_group_id` 加嵌套 `resource_spec_price`（其内含 `quota_id`）传达；`framework_config` 顶层**没有** `quota_id` 字段（proto 会以 `unknown field "quota_id"` 拒绝整个请求），这一点与 HPC 的 `slurm_cluster_spec.predef_quota_id`、Ray/notebook 的扁平 `quota_id` 都不同。`exclude_nodes` 是“排除这些 Ready 节点”，候选来自 `ListNodeDimension`，不是 pin 到节点；它必须放在 create payload 顶层，放入 `framework_config[0]` 会被当前 v2 proto 以 `unknown field "exclude_nodes"` 拒绝。 | `browser_api.jobs.create_training_job`；`inspire job create --exclude-node <NODE>` / `job batch` |
...
| `POST` | `/api/v2/train?Action=ListJobs` | 当前前端列表页 v2 入口。body 支持顶层 `workspace_id`、`page_num`、`page_size`、`created_by`、`keyword`、`status`；`filter` / `filter_by` / `page` / `name` / `statuses` 会被 proto 拒。响应 `Result.jobs[]` / `Result.total`，job row 字段与 v1 list 兼容并额外含 `exclude_nodes`、`node_infos`、`timeline` 等。`inspire job list --show-ids` 可显式保留每行 `job_id`；随后 Job-specific 命令可直接传该 ID，跳过 name resolver 的额外 ListJobs 请求。 | `browser_api.jobs.list_jobs`；`inspire job list` / name resolver |
...
- 2026-07-22 live qz SPA 从 `GET /api/v1/user/routes/{workspace_id}` 的 `userWorkspaceList.routes[].is_fair_workspace`（JSON boolean）选择写入合同。值为 `true` 时，共享逻辑（`/assets/utils.I5ItUnz7.js`）只生成 `task_priority=1` 或 `task_priority=4`：项目优先级小于 4 时写 1，否则写 4；对应公平调度语义下的 `1=LOW`（可抢占）、`4=HIGH`（稳定）。值为 `false` 或缺失时仍使用 `1–10`。这不表示后端枚举会拒绝其他历史值或直接请求值。
...
Jupyter terminal REST + WebSocket 是受限 notebook 的命令执行 transport。SSH bootstrap 仍复用这些底层能力，但在 `public_internet=false` 的 notebook 上 CLI 不再启动 SSH/rtunnel；`exec` / `shell` 使用 JupyterTerminal。

一次 `exec` 的当前链路是：优先直接打开 `{prefix}/notebook/lab/{notebook_id}` 取得同源 Jupyter surface，通过 `api/terminals` 创建本次调用专属 terminal，再连接 `terminals/websocket/{name}` 收发 PTY 消息，最后只删除本次创建的 terminal。完整 `/ide?notebook_id=...` 页面只作为直接 Lab 入口失败时的兼容 fallback；命令执行不等待 JupyterLab 的本地化“加载中”UI 文案。完成标记必须包含完整 `marker:exit:<code>` 记录后才能关闭 WebSocket，避免标记和退出码跨 frame 时丢失状态。

Transport policy 对平台已明确不可上网的 H100 / H200 类型直接选择 JupyterTerminal，不再先运行一次 live network probe；可能联网或类型未知的 Notebook 仍通过 `net-test` 同款 probe 判断是否允许 SSH。显式 `--account` 时，Notebook 解析、Lab 打开、terminal REST / WebSocket 和 Playwright Proxy 必须复用同一个目标账号 Web Session。
