...
| `POST` | `{prefix}/train_job/instance_list` | 该任务的 pod 实例。平台 body: `{jobId, page_num, page_size}`；CLI 封装用 `num` 映射到第一页 `page_size`。 | `browser_api.jobs.list_job_instances`；`inspire job instances --workspace <name> --num <N>` / `job shell` / `job logs` |
...
| `POST` | `{prefix}/hpc_jobs/instances/list` | HPC pod / component 实例列表。body: `{jobId, page_num, page_size}`；返回 `{items,total}`，单条含 `component`、`name`、`node`、`status`、`created_at`、`started_at`、`finished_at`、`running_time_ms` | `browser_api.hpc_jobs.list_hpc_job_instances` |
