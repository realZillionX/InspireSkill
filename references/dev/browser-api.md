...
| --- | --- | --- | --- | --- |
| **OpenAPI** | `/openapi/v1` | Bearer token (username/password → `/auth/token` 换得) | [`platform/openapi/`](../../cli/inspire/platform/openapi/) | 公开合约，**10 条业务端点**：train_job / hpc_jobs / inference_servings 的 `create / detail / stop` + `cluster_nodes/list` |
...
- 能在 OpenAPI 上做的就走 OpenAPI（`job` / `hpc` 的 create / detail / stop；`inference_servings` 的 create / detail / stop 在 helper 层可用，其中 CLI 只暴露 `serving status/stop`）—— 稳、无头浏览器开销。`notebook status` 只走 Browser API。
...
Agent 默认看人类输出；结构化输出只用于脚本。平台的 `list` 端点会把最新版本属性合入模型条目，而 `detail` 端点只返回模型主记录，所以 `is_vllm_compatible` 在 `list` 与 `status` 中可能不一致。要准确的版本属性，用 `inspire model versions <model-id>`。
...
OpenAPI 有 `create / detail / stop` 3 条（见 [openapi.md](openapi.md) 第 3 节）；**列表 / 配置 / 用户 / 项目** 只能走 Browser API。CLI 组合二者暴露 `inspire serving list/status/stop/configs/metrics`；`create` 参数过多（`model_id / port / replicas / ...`）暂不暴露为 CLI 子命令，但 OpenAPI helper 层已有封装。
