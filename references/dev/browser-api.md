...
Serving 连句柄都不叫 `job_id`，是第一次拿 `job_id` 去打才发现的。所以控制台那个按 Workload 重映射参数的动作是必需的，不是风格；`build_remote_cmd_ws_url` 对表里没有的 Workload 直接抛错而不是按类比猜。
