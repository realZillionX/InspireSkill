...
- `CLI 引用` 列优先指向当前仓库中的 wrapper、helper 或用户命令；尚未封装但合同已经用 live probe / Chrome 抓包闭合的端点，可以标为 `未封装`，但必须写明已验证的请求体、响应形状和不封装原因。
- 受限端点可以进入清单，但只能写已验证行为。例如普通用户固定返回 `403 user is not system admin`，就标为 `受限`，不能推导 admin 成功响应。
...
| `GET` | `{prefix}/user/{user_id}` | 指定用户详情。实测普通用户可读当前登录用户自己的记录，返回字段与 `/user/detail` 同构：`id`、`name`、`name_en`、`email`、`avatar_url`、`global_role`、`extra_info`、`created_at` | `browser_api.users.get_user_detail` |
...
| `POST` | `{prefix}/ssh/list` | 当前用户 SSH 公钥列表。body: `{page, page_size}`；返回 `{list, total}`，单条含 `id` / `ssh_id`、`name`、`fingerprint` 等。2026-05-09 Chrome 页面和 live probe 对齐 | `browser_api.users.list_user_ssh_keys`；`inspire user ssh-keys list` |
...
| `POST` | `{prefix}/user/list` | 管理员用户列表。普通账号用 `{}`、分页 body 或 filter body 均返回 `403` / `code:100004` / `user is not system admin`；不能作为普通 CLI 的用户选择来源 | 受限；不封装为默认 CLI |
...
| `POST` | `{prefix}/notebook/users` | notebook 页面用户筛选下拉。body: `{workspace_id}`；返回 `{list,total}`，单条为用户 metadata | `browser_api.notebooks.list_notebook_users` |
...
| `POST` | `{prefix}/train_job/instance_list` | 该任务的 pod 实例。平台 body: `{jobId, page_num, page_size}`；CLI 封装用 `num` 映射到第一页 `page_size`。 | `browser_api.jobs.list_job_instances`；`inspire job instances <name> --workspace <workspace> --num <N>` / `job shell` / `job logs` |
...
| `POST` | `{prefix}/hpc_jobs/instances/list` | HPC pod / component 实例列表。body: `{jobId, page_num, page_size}`；CLI 封装用 `num` 映射到第一页 `page_size`。返回 `{items,total}`，单条含 `component`、`name`、`node`、`status`、`created_at`、`started_at`、`finished_at`、`running_time_ms` | `browser_api.hpc_jobs.list_hpc_job_instances`；`inspire hpc instances <name> --workspace <workspace> --num <N>` |
...
| `POST` | `{prefix}/ray_job/instances/list` | pod 级视图：head + 每个 worker 组的实际 pod。body: `{ray_job_id, page_num, page_size}`；CLI 封装用 `num` 映射到第一页 `page_size`。返回 `items[]` 每条含 `instance_id` / `instance_type` (`head` / `worker`) / `worker_group_name` / `status` (`pending` / `running` / ...) / `cpu_count` / `memory_size` / `gpu_count` / `priority` / `created_at` | `browser_api.ray_jobs.list_ray_job_instances`；`inspire ray instances <name> --workspace <workspace> --num <N>` |
...
| `POST` | `{prefix}/model/users` | 模型页用户筛选下拉。body: `{project_id}`；返回 `{list,total}`，单条含 `user_id` / `user_name` | `browser_api.models.list_model_users` |
...
下列模型写操作已经从 SPA bundle 解析出路径和 payload，但当前 CLI 没有用户命令封装，维护时不要把它们当作只读 helper 使用：
...
网页端部署表单已实测确认：当前 CLI 用户合同覆盖“自定义部署”（`inference_serving_type:"CUSTOM"`）。自定义部署使用注册模型、镜像 `mirror_id`、运行命令、端口、优先级、资源规格、副本数、单副本实例数和共享内存。`resources specs --usage serving` 必须查 `SCHEDULE_CONFIG_TYPE_SERVE`，不是训练任务的 `SCHEDULE_CONFIG_TYPE_TRAIN`。在 `H200-2号机房` 实测到的 serving 规格是 `1,18,200`、`2,36,400`、`4,72,900`、`8,168,1800`。
