...
运行环境跑通后要保存成镜像；每次保存前必须按 [`references/notebook.md`](references/notebook.md#6-基底环境与保存镜像) 检查待固化内容，彻底清理所有多余垃圾文件，确认无残留且环境仍可用后再保存。保存是 notebook 的生命周期事件，不是镜像目录操作：`notebook save-image` 会触发一段中等时长的镜像保存过程，过程中不可操作该 notebook；保存完毕后 notebook 不会被自动停止，仍可继续连接和使用。保存出的镜像才是后续 notebook / job / HPC / Ray / serving 应复用的稳定环境。
...
`INSPIRE.md` 不是仓库级必备元数据，只在仓库需要跨 Agent / 成员 / 会话复用稳定远端路径、镜像、模型、数据版本或永久基础设施时才创建。每个资产条目自带 Project / Workspace 适用范围，同一文件可覆盖多个 Project。InspireSkill 本身是 CLI、Skill 与文档的通用工具源仓库，不对应业务资产，因此根目录不应存在 `INSPIRE.md`。通用文档只说明该合同的边界，不复制任何具体账号的路径、资产 ID 或运行状态。
