...
- **Session 过期后的自动登录会被多层重试放大，足以触发 CAS 账号锁定。** 一次 401/302 原先先拿同一份失效 Cookie 再走浏览器请求；并发 CLI 与后台缓存进程只会复用一次**成功**刷新，第一个登录失败后所有等待者仍会逐个提交凭据；`init` 和资源可用性查询还各自包了一层登录重试；requests/CAS 在已经提交凭据后遇到 5xx 或响应丢失，又会回落 Playwright 再提交一次。

  现在 401/302 禁止重定向并直接进入唯一的 Session 续期边界，最多重建一次；凭据一旦提交，任何未完成结果都不再自动回落另一条登录通道。失败会按 Account Alias 写一个不含凭据和平台响应的本地熔断标记，**一分钟内**前台命令、后台刷新与其它进程全部只在本地报错，不再访问 CAS；修改该账号的 `config.toml`、成功写入更新的 Session 或等待一分钟都会解除。`init` 与资源查询的第二层重试同时删除。

  验收没有连接真实平台：假 401/302 锁定为一次续期且不经过浏览器，CAS 503 锁定为一次凭据提交且不回落；8 个独立进程同时撞上一份失效 Session，登录成功时共享一次刷新，登录失败时同样只有第一个进程提交一次，其余 7 个命中本地熔断。
