...
```bash
# 1. 建账号目录。沿途问：平台 username（默认=<name>）/ password（二次确认）/
#    base URL（默认 https://qz.sii.edu.cn）/ 代理（有提示典型值）/ 是否设为活动账号。
#    想跳过交互：加 --non-interactive 并把所有字段都用 flag 给齐。
inspire account add <name>

# 2. 登录平台、发现可用项目 + workspace + compute group，让你选存储池
#    （hdd / ssd / qb-ilm / qb-ilm2）作为远端工作目录。写入 ~/.inspire/accounts/<name>/config.toml。
inspire init --discover

# 3. 核对生效配置（来源标注哪个字段从 default / account / project / env 层来）
inspire config show --compact

# 4. 看实时空余（GPU / CPU），确认平台能连通
inspire resources list --all --include-cpu
```

多账号场景就再 `inspire account add <name2>`，`inspire account use <name>` 切换。从旧版（`[accounts."<user>"]` 布局）升级跑一次 `inspire account migrate`，它会把老 `~/.config/inspire/config.toml` + `~/.inspire/bridges-*.json` + `~/.cache/inspire-skill/web_session-*.json` 原地搬到 `~/.inspire/accounts/<name>/` 下，并把原件备份到 `~/.inspire/legacy-<timestamp>/`。
