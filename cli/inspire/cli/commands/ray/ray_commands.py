...
def start_ray(ctx: Context, name: str, workspace: str, pick: Optional[int]) -> None:
    ...
    The platform keeps the head and worker-group spec on the record, so a job stopped with `inspire ray stop` comes back with the same cluster shape and driver command; nothing has to be re-specified.

    Only a stopped job can be started; the command reports what the job's status actually became rather than what the platform answered. Follow the startup with `inspire ray events <name> --workspace <workspace>`.
    """
    name = _reject_ray_name_at_boundary(ctx, name)
    try:
        config, _ = Config.from_files_and_env(require_credentials=False)
...
def stop_ray(ctx: Context, name: str, workspace: str, pick: Optional[int]) -> None:
    ...
    The record survives; `inspire ray start <name>` brings the same cluster back. Use `inspire ray delete` to remove the record entirely.
    """
    name = _reject_ray_name_at_boundary(ctx, name)
    try:
        config, _ = Config.from_files_and_env(require_credentials=False)
...
def events_ray(
    ...
    Critical for diagnosing stuck PENDING jobs — the `FailedScheduling` events spell out exactly why the scheduler can't place a pod (insufficient CPU / GPU, node affinity mismatch, taint, etc.). Cluster events and every pod's events arrive in one timeline with an `Instance` column; `--instance` narrows to one role and `--workload-level` keeps only the controller's half.
...
def instances_ray(
    ...
    NAME is resolved within the explicit workspace and current live user. Shows each pod's status; check `inspire ray events <name> --workspace <workspace>` for scheduler reasons when pods remain pending.
    """
    try:
...
def delete_ray(ctx: Context, name: str, workspace: str, yes: bool, pick: Optional[int]) -> None:
    ...
    The entry disappears from the platform Ray list. This cannot be undone; if the job is still running, `stop` it first so the scheduler releases reserved capacity cleanly.
    """
    name = _reject_ray_name_at_boundary(ctx, name)
    require_confirmation(
        ctx,
...
def shell_ray(
    ...
    Defaults to the head, which runs the driver and is where `ray status` and the cluster's own logs live. Pick a worker by its Role / Rank when the question is about one group's processes rather than the cluster's.
