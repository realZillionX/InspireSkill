    ...
    for row in rows:
        ...
        for key, _header, _align in columns:
            ...
            if key.endswith("usage_rate"):
                ...
                cells.append(display_name(value, fallback="-"))
...
def usage_resources(
    ...
    try:
        ...
        if group:
            matched_groups = resolve_group_ids(
                session=session,
        ...
        if mode == "mine":
            ...
            fair_scheduling = fair_scheduling_or_unknown(session, workspace_id)
            if mode == "task":
                rows = task_rows(tasks, workspace=label)
            else:
                rows = project_user_rows(
                    tasks,
                    workspace=label,
        ...
        if ctx.json_output:
            ...
            if matched_groups:
                payload["compute_groups"] = [
                    display_name(name) for _group_id, name in matched_groups
                ]
        ...
        if not rows:
            if mode == "mine":
                ...
                names = ", ".join(display_name(name) for _gid, name in matched_groups)
