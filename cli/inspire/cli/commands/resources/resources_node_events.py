...
def node_events(
    ...
    \b
    Read node names off the `Node` column of `inspire job instances`,
    `inspire hpc instances`, or `inspire notebook status`. Several nodes
    answer in one merged timeline with a `Node` column. A node the cluster
    does not know is not an error — it simply has no events, so check the
    spelling before reading silence as a healthy node.
