...
def serving_instance_views(
    ...
    from inspire.services.serving.serving_views import public_serving_instances

    views: list[ServingInstanceView] = []
    rows = list(instances)
    for public, raw in zip(public_serving_instances(rows), rows):
...
def select_serving_instance_views(
    views: Sequence[ServingInstanceView],
...
def fetch_serving_instances(
    serving_id: str, *, session: WebSession | None = None,
) -> tuple[list[dict[str, Any]], int]:
    """Fetch instances with the same total-based expansion as CLI --all."""
    items, total = api.list_serving_instances(
        serving_id, page=1, page_size=200, session=session
    )
    if total > len(items):
        items, expanded_total = api.list_serving_instances(
            serving_id, page=1, page_size=max(total, len(items), 1), session=session
        )
