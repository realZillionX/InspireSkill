...
def _model_label(item: object) -> str:
    ...
    version = _value(item, "model_version") or (
...
def _image_label(item: object) -> str:
    ...
    if name:
        text = sanitize_public_text(name, omit_urls=True)
        if not text or str(name).strip().lower().startswith(("http://", "https://", "docker://")):
            return ""
        return text
...
def public_serving_list_item(
    ...
    *,
    ...
    workspace = sanitize_public_text(fallback_workspace, omit_urls=True) or str(
        view.get("workspace") or ""
    )
    return {
        "name": view.get("name", ""),
...
def public_serving_instance(item: object, *, index: int) -> dict[str, Any]:
    ...
    if not resource:
        ...
        memory = spec.get("memory_size_gib") or spec.get("memory_gib") or spec.get("memory_size")
        if cpu not in (None, ""):
        ...
        _compact(
            {
                ...
                "memory_gib_min": item.get("memory_size_min") or item.get("memory_size_gib_min"),
                "memory_gib_max": item.get("memory_size_max") or item.get("memory_size_gib_max"),
