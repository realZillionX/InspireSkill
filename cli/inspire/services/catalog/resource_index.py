...
from inspire.platform.web.flow import blocking_io, blocking_call, perform_sync, enter_context, exit_context
from inspire.services.utils import json_formatter
from inspire.services.utils.processes import process_is_alive
...
class ResourceIndex:
    ...
    def _is_corruption_error(error: BaseException) -> bool:
        if isinstance(error, _CorruptResourceIndexError):
            return True
    ...
    def refresh_lease(
        ...
        *,
        ...
        """Offload only lease I/O; the caller's fetch stays on its own driver."""
        lease = self._refresh_lease(
            scope, lease_seconds=lease_seconds, holder=holder,
            now=now, raise_on_error=raise_on_error,
        )
        acquired = perform_sync(blocking_call(enter_context, lease))
        try:
            yield acquired
        finally:
            perform_sync(blocking_call(exit_context, lease, None, None, None))
    ...
    @blocking_io
    def clear(self, resource_types: Iterable[str] | None = None, *, base_url: str = "") -> int:
        ...
        with self._connect() as connection:
            ...
            if selected:
                ...
                clauses.append(f"resource_type IN ({placeholders})")
                params.extend(selected)
            if base_url:
                clauses.append("base_url = ?")
                params.append(base_url.rstrip("/"))
            where = " WHERE " + " AND ".join(clauses) if clauses else ""
            removed = connection.execute(
                f"DELETE FROM resource_identity{where}", params
            ).rowcount
            connection.execute(f"DELETE FROM resource_scope{where}", params)
